Chapter 8. 애플리케이션에서 파드 메타데이터와 그 외의 리소스에 액세스하기
---

이번 장에서 알아보는 주제
1. 특정 파드와 컨테이너 메타데이터를 노출하는 방법
...
## 쿠버네티스 API 서버와 통신하기
downward API를 사용하면 파드/컨테이너의 메타데이터를 그 안에서 실행되는 프로세스로 쉽게 전달 할 수 있는데, 대신 사용 가능한 메타데이터가 한정적이다. 
